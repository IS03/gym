...
export async function getGlobalCalendar(input: {
  ...
  if (end < start) return buildGlobalCalendarDays({ grid, dayLogs: [], meals: [], workouts: [], metrics: [], bodyMeasurementDates: [] });
  ...
  const [mealsResult, workoutsResult, metricsResult, bodyResult] = await Promise.all([
    ids.length
    ...
    ids.length
      ...
      : Promise.resolve({ data: [], error: null }),
    supabase.from("daily_metric_values").select("metric_date,value").eq("user_id", userId).gte("metric_date", start).lte("metric_date", end),
  ...
  if (metricsResult.error) throw new Error(`Leer métricas del calendario: ${metricsResult.error.message}`);
  ...
  return buildGlobalCalendarDays({
    ...
    metrics: (metricsResult.data ?? []) as GlobalCalendarMetricFact[],
