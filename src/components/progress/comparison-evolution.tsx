...
export function ComparisonEvolution({
  ...
  const referenceValues = useMemo(() => result?.alignedSeries.map((point) => point.reference?.value ?? null) ?? [], [result]);
  ...
  return <div className="space-y-4">
    ...
    <div className="flex flex-wrap gap-x-4 gap-y-1 text-xs text-muted-foreground">
      ...
      <span className="flex items-center gap-1.5"><span className="w-5 border-t-2 border-dashed border-muted-foreground" />{referenceLabel}</span>
    </div>
    {result.eligibility.status !== "comparable" ? <p className="rounded-xl border bg-muted/20 px-4 py-6 text-sm text-muted-foreground">No hay suficientes datos para graficar esta comparación.</p> : <>
      <svg className="block h-auto w-full overflow-visible" viewBox={`0 0 ${WIDTH} ${HEIGHT}`} role="group" aria-label={`Evolución comparada de ${result.metric.label}`} style={{ touchAction: "pan-y" }}>
        ...
        {referenceSegments.map((segment, index) => <polyline key={`b-${index}`} points={segment.map((point) => `${point.x},${point.y}`).join(" ")} fill="none" className="text-muted-foreground" stroke="currentColor" strokeWidth="2" strokeDasharray="5 4" strokeLinecap="round" strokeLinejoin="round" />)}
      ...
      <ChartDetail title={relativeLabel(selectedIndex, report.bucket)} items={[
        ...
        { label: referenceLabel, value: formatProgressMetricValue(referenceValues[selectedIndex] ?? null, result.metric) },
      ]} className="min-h-20" />
    </>}
  </div>;
}
