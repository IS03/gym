...
import { formatProgressMetricValue, PROGRESS_CUSTOM_RANGE_MAX_DAYS, type ProgressPeriodPreset, type ProgressPeriodRange } from "@/lib/progress/analytics";
import {
...
function rangeLabel(range: ProgressPeriodRange) {
  return `${shortDate(range.start)}–${shortDate(range.end)}`;
}

function PeriodButton({ analysis, reference, today }: { analysis: TrainingAnalysis; reference: ProgressTemporalComparisonReference; today: string }) {
  ...
  return <>
    <button type="button" onClick={() => { setDraftRange({ start: analysis.range.start, end: analysis.range.end }); setOpen(true); }} className="flex min-h-11 w-full items-center gap-3 rounded-xl border bg-card px-3 py-2 text-left outline-none focus-visible:ring-2 focus-visible:ring-ring" aria-expanded={open}>
      ...
      <span className="min-w-0 flex-1"><span className="block text-sm font-semibold">{analysis.range.label}</span><span className="block truncate text-xs text-muted-foreground">{rangeLabel(analysis.range)} · vs. {rangeLabel(reference.period)}</span></span>
...
export function TrainingGeneralV2({
  ...
  return <div className="space-y-7">
    <div className="space-y-2"><PeriodButton analysis={analysis} reference={reference} today={today} /><ComparisonConfigurator metrics={LOAD_METRIC_OPTIONS} primaryPeriod={analysis.range} today={today} selectedMetricKeys={analytics.loadComparison.selectedMetricKeys} referenceType={comparisonQuery.referenceType ?? "previous_period"} referencePreset={(comparisonQuery.referencePreset ?? null) as ProgressPeriodPreset | null} referencePeriod={reference.period} initialView={comparisonQuery.initialView} activeMetricKey={analytics.loadComparison.activeMetricKey} viewParam="comparisonView" triggerLabel="Ajustar comparación" triggerClassName="mt-0" showViewSelection={false} />{comparisonError ? <p className="text-xs text-destructive">{comparisonError}</p> : null}</div>
