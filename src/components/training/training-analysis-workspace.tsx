...
import { trainingComparisonKindForView, type TrainingComparison } from "@/lib/phase2/training-comparison";
...
function ComparisonAction({ state, label, a }: { state: WorkspaceState; label: string; a?: string | null }) {
  return <Link href={trainingAnalysisComparisonPath(state, trainingComparisonKindForView(state.view), { a })} className="inline-flex min-h-10 items-center text-sm font-medium text-primary hover:underline">Comparar {label}</Link>;
}
...
export function TrainingAnalysisWorkspace({ analysis, view, routineId, muscleKey, exerciseQuery, exerciseRoutineId, exerciseMuscleKey, comparison }: { analysis: TrainingAnalysis; view: TrainingAnalysisView; routineId: string | null; muscleKey: string | null; exerciseQuery?: string; exerciseRoutineId?: string | "all"; exerciseMuscleKey?: string | "all"; comparison?: TrainingComparison | null }) {
  const state: WorkspaceState = { view, period: analysis.period, routineId, muscleKey, exerciseQuery, exerciseRoutineId, exerciseMuscleKey, comparison: comparison?.kind, comparisonA: comparison?.a?.id, comparisonB: comparison?.b?.id };
