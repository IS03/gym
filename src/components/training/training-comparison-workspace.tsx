...
function ComparisonSelectors({ comparison, state }: { comparison: TrainingComparison; state: TrainingAnalysisNavigationState }) {
  ...
  const update = (side: "a" | "b", value: string) => {
    ...
    router.push(trainingAnalysisComparisonPath(state, comparison.kind, { a, b }));
  };
  if (comparison.kind === "periods") {
    return <div className="grid grid-cols-2 gap-2" aria-label="Períodos comparados"><div className="rounded-lg border border-primary/30 bg-primary/8 px-3 py-2.5"><p className="text-[11px] font-semibold uppercase tracking-[0.1em] text-primary">A · Actual</p><p className="mt-1 text-sm font-medium">{dateRange(comparison.rangeA)}</p></div><div className="rounded-lg border border-dashed px-3 py-2.5"><p className="text-[11px] font-semibold uppercase tracking-[0.1em] text-muted-foreground">B · Anterior</p><p className="mt-1 text-sm font-medium">{dateRange(comparison.rangeB)}</p></div></div>;
  }
...
function ComparisonSummary({ comparison }: { comparison: TrainingComparison }) {
  ...
  return <section className="space-y-3" aria-label="Resumen comparativo"><div className="grid grid-cols-[minmax(0,1fr)_minmax(0,1fr)_minmax(0,1fr)] gap-2 text-xs"><span aria-hidden /><p className="truncate text-right font-semibold text-primary">A · {comparison.a.label}</p><p className="truncate border-l border-dashed pl-2 text-right font-semibold text-foreground">B · {comparison.b.label}</p></div><dl className="overflow-hidden rounded-xl border divide-y divide-border/70">{comparison.metrics.map((metric) => {
    ...
    const deltaLabel = delta.absolute === null
      ...
      : delta.absolute === 0
        ? "Sin cambios"
        : `A − B: ${signedMetricValue(delta.absolute, metric)}${delta.percentage === null ? delta.hasComparableBaseline ? "" : " · sin base porcentual" : ` · ${delta.percentage > 0 ? "+" : ""}${number(delta.percentage, 1)}%`}`;
    return <div key={metric} className="grid grid-cols-[minmax(0,1fr)_minmax(0,1fr)_minmax(0,1fr)] gap-2 px-3 py-3"><dt className="self-center text-xs text-muted-foreground">{METRIC_LABELS[metric]}</dt><dd className="metric-number min-w-0 text-right text-sm font-semibold">{metricValue(a, metric)}</dd><dd className="metric-number min-w-0 border-l border-dashed pl-2 text-right text-sm font-semibold">{metricValue(b, metric)}</dd><p className="col-span-3 text-xs text-muted-foreground">{deltaLabel}</p></div>;
  })}</dl></section>;
}
...
export function TrainingComparisonWorkspace({ comparison, state, backLabel }: { comparison: TrainingComparison; state: TrainingAnalysisNavigationState; backLabel: string }) {
  ...
  return <div className="space-y-6"><section className="space-y-4"><Link href={exitHref} className="inline-flex min-h-10 items-center gap-1 text-sm font-medium text-primary hover:underline"><ArrowLeft className="size-4" aria-hidden />{backLabel}</Link><div><h2 className="text-xl font-semibold tracking-tight">{comparison.title}</h2><p className="mt-1 text-sm text-muted-foreground">Valores históricos de sesiones finalizadas. La diferencia se expresa como A − B.</p></div><ComparisonSelectors comparison={comparison} state={state} /></section>{!hasPair ? <EmptyState>Necesitás al menos dos opciones distintas para comparar en este período.</EmptyState> : !hasAnyData ? <EmptyState>No hay sesiones finalizadas para comparar entre estos dos contextos.</EmptyState> : <><ComparisonSummary comparison={comparison} /><ComparisonChart comparison={comparison} /></>}</div>;
}
