...
export default async function TrainingProgressPage({
  ...
  const musclesData = view === "muscles"
    ...
    : null;
  const exercisesData = view === "exercises"
    ? await getTrainingExercisesAnalysis(period, temporalReference, {
      selectedMetricKeys: progressQuery.selectedMetricKeys,
      activeMetricKey: progressQuery.activeMetricKey,
    }, customRange)
    : null;
  const analysis = generalData?.current ?? routinesData?.current ?? musclesData?.current ?? exercisesData?.current ?? await getTrainingAnalysis(period, customRange);
  ...
  const comparison = view === "exercises" && comparisonKind === "previous" && exercisesData
    ? buildTrainingSelfComparison({ analysis, previousAnalysis: exercisesData.reference, subjectType, subjectId })
