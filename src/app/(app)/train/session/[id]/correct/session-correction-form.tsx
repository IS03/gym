...
export function SessionCorrectionForm({ detail, returnHref }: { detail: WorkoutSessionDetail; returnHref?: string | null }) {
  ...
  const sessionHref = `/train/session/${detail.session.id}${returnHref ? `?${new URLSearchParams({ return: returnHref }).toString()}` : ""}`;
  ...
  return <div className="space-y-5">
    ...
    {error ? <p className="text-sm text-destructive" role="alert">{error}</p> : null}{saved ? <div className="flex flex-wrap items-center gap-3"><p className="text-sm font-medium text-emerald-700 dark:text-emerald-300">✓ Correcciones guardadas</p><Link href={sessionHref} className="inline-flex min-h-11 items-center text-sm font-medium text-primary hover:underline">Volver a la sesión</Link></div> : null}<Button className="w-full sm:w-auto" type="button" disabled={pending} onClick={save}>{pending ? "Guardando…" : "Guardar correcciones"}</Button>
  </div>;
}
