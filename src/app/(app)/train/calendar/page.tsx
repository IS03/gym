...
export default async function TrainCalendarPage({ searchParams }: { searchParams?: Promise<Record<string, string | string[] | undefined>> }) {
  ...
  const month = (typeof sp.month === "string" ? sp.month : today.slice(0, 7)) as `${number}-${number}`;
  ...
  return <div className="space-y-5 lg:mx-auto lg:max-w-5xl">
    ...
    <Card size="sm" className="surface-elevated">
      ...
      <CardContent className="pb-0">
        <div className="grid grid-cols-7 gap-1 text-center text-xs text-muted-foreground sm:gap-2">{["L", "M", "X", "J", "V", "S", "D"].map((day) => <div key={day} className="py-0.5">{day}</div>)}</div>
        ...
        <p className="mt-2.5 text-xs text-muted-foreground">Borde resaltado = día con al menos una sesión terminada.</p>
      </CardContent>
    </Card>
  </div>;
}
